...
--Review: group function  ---- max,min,count,avg,sum

--show the number of products for categary 2,3,5
...
--find categaries with number of products more than 100
...
select hire_date,to_char(hire_date,'DAY')
from employees;
select hire_date,to_char(hire_date,'DD')
from employees;
...
select credit_limit          
from customers
where customer_id = 178;         --5000

-- then you select all customers that their credit limit >= 5000
select customer_id, name, credit_limit
from customers
where credit_limit >= 5000
order by credit_limit;
...
select customer_id, name, credit_limit
from customers
where credit_limit >= (select credit_limit          
from customers
where customer_id = 178)              --5000
order by credit_limit;
...
-- is not allowed to be used here. It can be used with ANY , ALL,IN

select customer_id, name, credit_limit
from customers
where credit_limit >=ALL(100,200,1400)             -- ANY                  
order by credit_limit;

-- Using IN operator to compare a column's value with multiple values returned by the subquery
select customer_id, name, credit_limit
from customers
where credit_limit IN (select credit_limit              ---IN   ,ANY  ,  ALL         
                        from customers
...
where credit_limit > ALL (select credit_limit
                        from customers
                        where customer_id in (244, 239, 35));
   --ANY:credit_limit >400 OR  credit_limit >2500 OR credit_limit >3700  
   --ALL:credit_limit >400 AND  credit_limit >2500 AND credit_limit >3700 
...
--how manycustomers with the maximum credit_limit
...
-- all any
-- find customers that their number of orders is graeter than the number of orders of all customer 4 and 18
